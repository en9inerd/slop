...
static bool uf_init(DupeResult *dr, int n) {
    ...
    if (!dr->parent) return false;
    for (int i = 0; i < n; i++) dr->parent[i] = i;
    return true;
}
...
void dupes_add_file(DupeResult *dr, const char *filepath, const char *content, size_t len) {
    ...
    for (int f = 0; f < scan.function_count; f++) {
        ...
        if (fi->body_offset + fi->body_len > len) continue;
...
void dupes_compute(DupeResult *dr, double threshold) {
    if (dr->func_count < 2) return;

    free(dr->parent);
    dr->parent = nullptr;
    dr->pair_count = 0;
    dr->cluster_count = 0;
    if (!uf_init(dr, dr->func_count)) return;
        ...
        for (int j = i + 1; j < dr->func_count; j++) {
            ...
            if (ncd < threshold) {
                if (dr->pair_count >= dr->pair_cap) {
                    dr->pair_cap *= 2;
                    dr->pairs = realloc(dr->pairs, (size_t)dr->pair_cap * sizeof(DupePair));
                }
    ...
    for (int i = 0; i < dr->func_count; i++) id_map[i] = -1;

    dr->cluster_count = 0;
    for (int i = 0; i < dr->func_count; i++) {
        int root = uf_find(dr->parent, i);
        if (id_map[root] < 0) id_map[root] = next_id++;
...
DupRatioResult dupes_compute_ratio(const DupeResult *dr) {
    ...
    if (dr->func_count > 0) {
        ...
        for (int i = 0; i < dr->func_count; i++)
            if (dr->funcs[i].cluster >= 0) cl_sz[dr->funcs[i].cluster]++;
        for (int i = 0; i < dr->func_count; i++)
            if (dr->funcs[i].cluster >= 0 && cl_sz[dr->funcs[i].cluster] > 1) out.funcs_in_dup++;
        free(cl_sz);
...
void dupes_free(DupeResult *dr) {
    for (int i = 0; i < dr->func_count; i++) free(dr->funcs[i].body);
    free(dr->funcs);
    free(dr->pairs);
    free(dr->parent);
    *dr = (DupeResult){};
