...
add_finding(SmellReport *r, SmellKind kind, SmellSeverity sev, int line,
            const char *fmt, ...) {
...
static void detect_comment_gradient(SmellReport *r, const ScanResult *s) {
  if (s->body_lines_top < MIN_BODY_LINES ||
      s->body_lines_bottom < MIN_BODY_LINES)
    return;
  int total_body_comments = s->body_comment_top + s->body_comment_bottom;
  if (total_body_comments < MIN_GRADIENT_CMTS)
    return;
...
/* Extracts named imports from TS/JS: handles { x, y as z }, * as ns, default
   imports. Skips `import type` (TS 3.8+) which doesn't introduce runtime
   bindings. */
...
/* Python: handles `from mod import x, y` and `import mod`. For `from` imports,
   captures the individual names; for bare `import`, captures the top-level
   module. */
...
static bool name_found_in_range(const ScanResult *s, const char *name,
...
/* Language-specific export/visibility rules: Go uppercase, TS/JS `export`
   keyword on the same or preceding line, Python leading underscore, C/C++
   `static` keyword. */
static bool func_is_exported(const ScanResult *s, const FuncInfo *fi) {
  ...
  if (s->specific == SPECIFIC_TS || s->specific == SPECIFIC_JS) {
    ...
    while (off > 0 && p[off - 1] != '\n')
      off--;
    while (off < fi->body_offset && (p[off] == ' ' || p[off] == '\t'))
      off++;
  ...
  if (s->lang == LANG_C_LIKE) {
    ...
    if (line_has_keyword(p, off, line_end, "static"))
      return false;
    if (off > 0) {
      ...
      while (prev_start > 0 && p[prev_start - 1] != '\n')
        prev_start--;
      if (line_has_keyword(p, prev_start, prev_end, "static"))
        return false;
    }
    return true;
  }
