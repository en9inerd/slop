...
static const ExtMap *find_entry(const char *filename) {
    ...
    for (const ExtMap *m = ext_table; m->ext; m++)
        if (strcmp(dot, m->ext) == 0) return m;
    return nullptr;
}
...
static const char *skip_exts[] = {".min.js", ".min.css", ".pb.go", ".generated.cs",
                                  ".pb.h",   ".pb.cc",   ".lock",  nullptr};

static bool has_skip_suffix(const char *name) {
    size_t nlen = strlen(name);
    for (int i = 0; skip_exts[i]; i++) {
        size_t slen = strlen(skip_exts[i]);
        if (nlen >= slen && strcmp(name + nlen - slen, skip_exts[i]) == 0) return true;
    }
    return false;
}

bool lang_is_source_ext(const char *filename) {
    if (has_skip_suffix(filename)) return false;
    const char *dot = find_ext(filename);
    if (!dot) return false;
    for (const ExtMap *m = ext_table; m->ext; m++)
        if (strcmp(dot, m->ext) == 0) return true;
    /* extra extensions not in ext_table but still source */
    static const char *extra[] = {".rb", ".lua", ".pl", ".pm", ".r", ".R", ".jl", nullptr};
    for (int i = 0; extra[i]; i++)
        if (strcmp(dot, extra[i]) == 0) return true;
    return false;
}
