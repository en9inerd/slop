...
static void load_slopignore(const char *dirpath) {
  ...
  while (fgets(line, sizeof(line), f) && s_ignore_n < MAX_IGNORE_ENTRIES) {
    ...
    while (l > 0 && (line[l - 1] == '\n' || line[l - 1] == '\r'))
      line[--l] = '\0';
    if (l > 0 && line[0] != '#')
      snprintf(s_ignore[s_ignore_n++], MAX_IGNORE_LEN, "%s", line);
  }
  fclose(f);
}
...
static bool matches_slopignore(const char *path) {
  for (int i = 0; i < s_ignore_n; i++) {
    ...
    while (*p) {
      ...
      if (seglen == elen && memcmp(p, s_ignore[i], (size_t)seglen) == 0)
        return true;
      p = *end ? end + 1 : end;
    }
  }
...
static bool is_git_repo(const char *dirpath) {
  char cmd[4200];
  snprintf(cmd, sizeof(cmd), "git -C \"%s\" rev-parse --git-dir 2>/dev/null",
           dirpath);
...
static void walk_git(FileList *fl, const char *dirpath) {
  ...
  while (fgets(line, sizeof(line), proc)) {
    ...
    if (l > 0 && line[l - 1] == '\n')
      line[--l] = '\0';
    if (!lang_is_source_ext(line))
      continue;
    if (matches_slopignore(line))
      continue;
    char full[8192];
