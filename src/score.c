...
static void json_load_gp(const char *json, const char *name, GaussParam *gp) {
    ...
    if (gp->sig_ai < MIN_GAUSS_SIGMA) gp->sig_ai = MIN_GAUSS_SIGMA;
    if (gp->sig_h < MIN_GAUSS_SIGMA) gp->sig_h = MIN_GAUSS_SIGMA;
}
...
bool calibration_load(Calibration *cal, const char *dir) {
    ...
    if (tmp.temperature < MIN_TEMPERATURE) tmp.temperature = DEFAULT_TEMPERATURE;
     ...
     */
    int valid_signals = 0;
    GaussParam *gps[] = {&tmp.regularity, &tmp.ccr,     &tmp.cond,  &tmp.dup, &tmp.git,
                         &tmp.ident,      &tmp.func_cv, &tmp.ttr,   &tmp.indent};
    for (int g = 0; g < 9; g++) {
        if (gps[g]->sig_ai > MIN_GAUSS_SIGMA && gps[g]->sig_h > MIN_GAUSS_SIGMA) valid_signals++;
        if (gps[g]->sig_ai < MIN_GAUSS_SIGMA) gps[g]->sig_ai = MIN_GAUSS_SIGMA;
        if (gps[g]->sig_h < MIN_GAUSS_SIGMA) gps[g]->sig_h = MIN_GAUSS_SIGMA;
    }

    if (valid_signals < 3) return false;

    double *bins[] = {&tmp.narr_p_ai,     &tmp.narr_p_h,     &tmp.decay_p_ai,   &tmp.decay_p_h,
                      &tmp.overwrap_p_ai, &tmp.overwrap_p_h, &tmp.namebrk_p_ai, &tmp.namebrk_p_h};
    for (int b = 0; b < 8; b++) {
        if (*bins[b] < MIN_BINARY_PROB) *bins[b] = MIN_BINARY_PROB;
    }
...
static double binary_llr(bool present, double p_ai, double p_h, double clamp) {
    if (present) return clampd(log(p_ai / p_h), -clamp, clamp);
...
static void add_detail(ScoreResult *out, const char *group, const char *signal, const char *meas,
                       double llr) {
    if (out->signal_count >= MAX_SIGNAL_DETAILS) return;
...
static double score_regularity(ScoreResult *out, const ScanResult *scan, const Calibration *cal,
                               double compression_ratio, double clamp) {
    int nz_count = 0;
    for (int i = 0; i < scan->line_length_count; i++)
        if (scan->line_lengths[i] > 0) nz_count++;

    double h_norm = 0;
    if (nz_count > 0) {
        int buckets[LINE_LEN_BUCKETS] = {};
        for (int i = 0; i < scan->line_length_count; i++) {
            if (scan->line_lengths[i] <= 0) continue;
            int b = scan->line_lengths[i] / LINE_LEN_BUCKET_W;
            if (b >= LINE_LEN_BUCKETS) b = LINE_LEN_BUCKETS - 1;
            buckets[b]++;
        }
        double h = 0;
        int used = 0;
    ...
    if (nz_count > 1) {
        ...
        if (var < 0) var = 0;
        if (mean > 0) cv = sqrt(var) / mean;
    }

    const double hn = 1.0 - clampd((h_norm - REG_ENTROPY_LO) / REG_ENTROPY_SPAN, 0, 1);
    ...
    if (has_compress) {
        const double rn =
            1.0 - clampd((compression_ratio - REG_COMPRESS_LO) / REG_COMPRESS_SPAN, 0, 1);
        reg = (rn + hn + cn) / 3.0;
    } else {
        reg = (hn + cn) / 2.0;
    }
    ...
    if (!has_compress) llr = clampd(llr, -LLR_CLAMP_NOCOMPR, LLR_CLAMP_NOCOMPR);
    if (scan->total_lines < MIN_LINES_DECAY) llr = clampd(llr, -LLR_CLAMP_SMALL, LLR_CLAMP_SMALL);

    char mbuf[48];
    snprintf(mbuf, sizeof(mbuf), "%.2f", reg);
    add_detail(out, "regularity", has_compress ? "composite" : "composite*", mbuf, llr);
    return llr;
}
...
static bool detect_decay(const ScanResult *scan) {
    if (scan->total_lines >= MIN_LINES_DECAY && scan->total_defects >= MIN_DEFECTS_DECAY) {
        int q_sz = scan->total_lines / 4;
        if (q_sz < 1) q_sz = 1;
        double rates[4];
        for (int q = 0; q < 4; q++) rates[q] = (double)scan->defects_per_quartile[q] / q_sz;
        if (spearman4(rates) > SPEARMAN_THRESHOLD) return true;
    }
    if (scan->body_lines_top >= MIN_BODY_LINES && scan->body_lines_bottom >= MIN_BODY_LINES) {
        ...
        if (dt / db > GRADIENT_THRESHOLD && total_c >= MIN_GRADIENT_CMTS) return true;
    }
    return false;
}
...
void score_compute(ScoreResult *out, const ScanResult *scan, const Calibration *cal,
                   double compression_ratio, double dup_ratio, const GitFeatures *git) {
    *out = (ScoreResult){};
    ...
    if (nc < NARRATION_TIER1) llr = binary_llr(false, cal->narr_p_ai, cal->narr_p_h, clamp);
    else if (nc < NARRATION_TIER2) llr = binary_llr(true, cal->narr_p_ai, cal->narr_p_h, clamp);
    else llr = clamp;
    snprintf(mbuf, sizeof(mbuf), "%s (%d hits)", nc >= NARRATION_TIER1 ? "yes" : "no", nc);
    ...
    if (scan->total_identifiers >= MIN_IDENTIFIERS) {
        ...
        snprintf(mbuf, sizeof(mbuf), "%.2f (%d/%d)", ispec, scan->generic_identifiers,
    ...
    if (scan->function_count >= MIN_FUNCS_CV) {
        ...
        for (int f = 0; f < scan->function_count; f++) {
            int lines = scan->functions[f].end_line - scan->functions[f].start_line + 1;
            if (lines < 1) continue;
