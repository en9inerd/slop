...
[[nodiscard]] double compress_ncd(const char *x, size_t xlen, const char *y, size_t ylen);
...
void dupes_add_file(DupeResult *dr, const char *filepath, const char *content, size_t len);
...
typedef struct {
    bool include_general_smells; /* include GENERAL-severity smells (default: false) */
    bool use_git;                /* include git commit pattern signal (default: false) */
    double prior;                /* P(AI) prior, 0.5 = uninformative (default: 0.5) */
    const char *calibration_dir; /* directory containing .slop-calibration.json, or nullptr */
} SlopOptions;
...
[[nodiscard]] SlopFileResult slop_analyze_file(const char *path, const SlopOptions *opts);
[[nodiscard]] SlopProjectResult slop_analyze_dir(const char *dirpath, const SlopOptions *opts);
