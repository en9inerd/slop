...
static bool is_conventional(const char *msg, int len) {
    static const char *prefixes[] = {"feat", "fix", "chore", "docs",  "style", "refactor",
                                     "test", "ci",  "perf",  "build", nullptr};
    for (int i = 0; prefixes[i]; i++) {
        int plen = (int)strlen(prefixes[i]);
        if (len < plen) continue;
        if (memcmp(msg, prefixes[i], (size_t)plen) != 0) continue;
        int j = plen;
        if (j < len && msg[j] == '(') {
            while (j < len && msg[j] != ')') j++;
            if (j < len) j++;
        }
        if (j < len && msg[j] == ':') return true;
        if (j + 1 < len && msg[j] == '!' && msg[j + 1] == ':') return true;
    }
    return false;
}
...
void git_features(GitFeatures *out, const char *filepath) {
    *out = (GitFeatures){};
    ...
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "git log -20 --format=\"%%B%%x00\" -- \"%s\" 2>/dev/null", filepath);
    ...
    if (status != 0 || buf_len == 0) { free(buf); return; }
    buf[buf_len] = '\0';
    ...
    while ((size_t)(p - buf) < buf_len) {
        ...
        int mlen = (int)(end - p);
        while (mlen > 0 && isspace((unsigned char)p[mlen - 1])) mlen--;
        ...
        if (mlen > 0) {
            ...
            if (is_conventional(p, mlen)) conventional++;
        }
        p = end + 1;
    }
