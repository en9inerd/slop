...
static void load_cal(Calibration *cal, const SlopOptions *opts, const char *fallback) {
    calibration_default(cal);
    if (opts->calibration_dir) (void)calibration_load(cal, opts->calibration_dir);
    else if (fallback) (void)calibration_load(cal, fallback);
}

static void apply_prior(ScoreResult *sc, const Calibration *cal, double prior_llr) {
    sc->raw_score += prior_llr;
    sc->probability = slop_sigmoid(sc->raw_score / cal->temperature);
}

static void analyze_content(SlopFileResult *r, const char *path, const char *content, size_t len,
                            const Calibration *cal, double dup_ratio, bool use_git,
                            double prior_llr, bool include_general) {
    ...
    GitFeatures gf = {};
    if (use_git) git_features(&gf, path);
...
SlopFileResult slop_analyze_file(const char *path, const SlopOptions *opts) {
    ...
    if (!content) { r.skipped = true; return r; }
...
SlopProjectResult slop_analyze_dir(const char *dirpath, const SlopOptions *opts) {
    ...
    for (int i = 0; i < fl.count; i++) {
        ...
        analyze_content(r, fl.paths[i], content, len, &pr.calibration, pr.dup_ratio, opts->use_git,
                        prior_llr, opts->include_general_smells);

        if (r->skipped) pr.skipped++;
        else if (r->probability >= PROB_FLAGGED) pr.flagged++;
        else if (r->probability >= PROB_SUSPICIOUS) pr.suspicious++;
        else pr.human++;
